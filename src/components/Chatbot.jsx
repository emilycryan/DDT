...
const Chatbot = ({ onNavigate }) => {
  ...
  const extractRecommendationsFromText = (text) => {
    const recommendationKeywords = [
      ...
      'assessment', 'program', 'physical activity', 'move more', 'meal plan', 'healthy foods', 'nutrition', 'lifestyle change'
    ];
...
- Mention lifestyle changes like healthy nutrition, physical activity, and avoiding tobacco
...
- In-person, hybrid,virtual, and on-demand program options`
      ...
      if (input.includes('diabetes')) {
        return "I can help you learn about diabetes prevention. Key steps include maintaining a healthy weight, building healthy eating habits, and staying physically active. Would you like to take our risk assessment to get personalized recommendations?";
      }
      if (input.includes('heart')) {
        return "Heart disease is preventable through lifestyle changes like regular physical activity, healthy eating, not smoking, and managing stress. Are you interested in learning about specific prevention strategies or finding a program to help?";
      }
  ...
  const handleQuickAction = async (action) => {
    ...
    if (action === "Take assessment for them") {
      ...
      setTimeout(() => {
        const routingMessage2 = {
          id: Date.now() + 2,
          text: `Navigating to the assessment now... This will help shape a prevention journey tailored for ${userState.careRecipientName ? `${userState.careRecipientName}'s` : 'their'} specific needs.`,
          sender: 'bot',
          timestamp: new Date()
        };
        setMessages(prev => [...prev, routingMessage2]);
    ...
    if (shouldRouteToAssessment(action)) {
      ...
      setTimeout(() => {
        const routingMessage2 = {
          id: Date.now() + 2,
          text: isForOthers ? 
            `Navigating to the assessment now... This will help shape a prevention journey tailored for ${recipientName ? `${recipientName}'s` : 'their'} specific needs.` :
            "Navigating to the assessment now... This will help us shape a prevention journey tailored just for you!",
          sender: 'bot',
          timestamp: new Date()
        };
        setMessages(prev => [...prev, routingMessage2]);
