...
export async function searchProgramsByLocation(zipCode, state, city, radius = 25) {
  try {
    const programs = await sql`
      ...
      WHERE 
        (pl.state = ${state} OR ${state} IS NULL)
        AND (pl.city ILIKE ${'%' + (city || '') + '%'} OR ${city} IS NULL)
        AND (pl.zip_code = ${zipCode} OR ${zipCode} IS NULL)
  ...
  try {
    ...
    const programs = await sql`
      ...
      WHERE 
        ...
        AND (pl.state = ${location.state} OR ${location.state} IS NULL)
      ORDER BY 
