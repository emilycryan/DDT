...
export async function searchProgramsByLocation(zipCode, state, city, radius = 25) {
  try {
    ...
    if (state) {
      whereConditions.push('pl.state = $' + (queryParams.length + 1));
      queryParams.push(state);
    }
    
    if (city) {
      whereConditions.push('pl.city ILIKE $' + (queryParams.length + 1));
      queryParams.push(`%${city}%`);
    }
    
    if (zipCode) {
      whereConditions.push('pl.zip_code = $' + (queryParams.length + 1));
      queryParams.push(zipCode);
    }
    
    const whereClause = whereConditions.length > 0 ? 'WHERE ' + whereConditions.join(' AND ') : '';
    
    const programs = await sql.unsafe(`
      SELECT 
        p.*,
        pl.address_line1,
        pl.address_line2,
